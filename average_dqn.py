...
class Agent(nn.Module):
    def __init__(
        ...
        t = lambda  : DuelingDQN(obs_dim=[self.obs_dim], hidden_dim=hidden_dim)

        self.policy_net.to(device)

        num_targets = 10
        self.target_nets = [t() for _ in range(num_targets)]
    ...
    def target_net(self,obs):
        q_vals = [target_net(obs) for target_net in self.target_nets]
        average_q = torch.stack(q_vals).sum(0)
        return average_q / len(self.target_nets)
    ...
    def act(self, obs, explore=True, average=False):
        ...
        if sample < self.epsilon() and explore:
            ...
            action = self.greedy(obs, average)
    ...
    def greedy(self, obs, average):
        with torch.no_grad():
            if average:
                logits = self.target_net(obs)
    ...
    def promote_target_net(self):
        ...
        to_update = self.train_iter % len(self.target_nets)
        target_net = self.target_nets[to_update]

        for target_param, param in zip(
            target_net.parameters(), self.policy_net.parameters()
        ):
...
def act(agents, obs_n, explore=True, average=False):
    actions = [agents[i].act(obs_n[i], explore, average) for i in range(len(agents))]
    return actions
...
def test(agents, env):
    act_test = partial(act, agents, explore=False, average=True)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "eps_start": 1,
        "eps_end": 0.01,
        "eps_decay": 50000,  # now on train iter not episode
        "eps_list": [],
        "gamma": 0.9,
        "capacity": 5000,
        "optim_iter": 1,
    ...
    cfg = {"notes": "Average, half hidden dim in downstream layers"}
    cfg.update(config)
