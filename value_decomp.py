...
class DQNOtmiser:
    ...
    def step()
        ...
        for agent in agents:
            ...
            action_values = logits.gather(1, action)
            next_logits = agent.target_net(next_obs)
            max_action_value = next_logits.max(1)[0].detach()
            targets = max_action_value * self.gamma + reward
            loss = self.loss_func(action_values, targets.unsqueeze(1))
...
class Trainer:
    ...
    def act(self, obs_n):
        return [agent.act(obs) for agent, obs in zip(agents, obs_n)]
...
def run(agents, env, epochs=EPISODES, thresh=100):
    ...
    for i in progress:
        ...
        agents, eps_reward, actions, num_done = run_episode(agents, env,train=True, explore=True, active_agent=None, show=False)
        ...
        if i % 500:
            ...
            wandb.log({"episode" : i, "test/num_home" : num_done_test, "test/reward" : test_reward , "test/rewards/max": max(train_rewards), "test/rewads/max_avg" :max(test_avg_rwrds)})
...
def test(agents, env, active_agent=None, show=False):
    for a in agents:
        a.train = False
    agents, eps_reward, obs, num_done_test = run_episode(agents, env, train=False, explore=False, show=show)
    for a in agents:
        a.train = True
    env.close()
...
env = gym.make("Switch4-v0", max_steps=ENV_STEPS) # Use "Switch4-v0" for the Switch-4 game
