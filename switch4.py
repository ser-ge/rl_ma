...
class DuelingDQN(nn.Module):
    def __init__(self, obs_dim, hidden_dim, num_actions=5):
        ...
        self.value_stream = nn.Sequential(
            nn.Linear(self.hidden, int(self.hidden/2)), nn.ReLU(), nn.Linear(int(self.hidden/2), 1)
        )
...
def train(
    ...
    train_iter = 0
    trainer = trange(num_episodes)
    env = gym.make(env)

    act_train = partial(act, agents, explore=True)
    act_test = partial(act, agents, explore=False)
    push = partial(push_to_agents_mem, agents,post_reward)

    optimiser = ValueDecomp(agents, capacity, gamma, lr)

    for episode_num in trainer:
        for obs_n, next_obs_n, actions, reward_n, done_counter, home_n, total_reward in episode(
            env, act_train
        ):
        ...
        if not episode_num % 50:
            ...
            *_, home_n_test, test_reward = last_result
            ...
            max_test_reward = max(train_rewards) if train_rewards else -20

            if test_reward > max_test_reward:
                save(agents)
                print(f"Saving: r={test_reward}, h={home_n_test}, iter={train_iter}")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "eps_list": [0.5, 0.2, 0.1, 0.01, 0.001],
        "gamma": 0.9,
        "capacity": 4000,
        "optim_iter": 5,
        "num_optim_updates": 2,
